...
class ListNewsAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        try{
            holder.author.setText(song.get("author"));
            holder.title.setText(song.get("title"));
            holder.time.setText(song.get("publishedAt"));
            holder.sdetails.setText(song.get("description"));

            if(song.get("urlToImage").toString().length() < 5)
            {
                holder.galleryImage.setVisibility(View.GONE);
            }else{
                Picasso.get()
                        .load(song.get("urlToImage"))
                        .resize(300, 200)
                        .centerCrop()
